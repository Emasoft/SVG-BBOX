  ...
  function printHelp() {
    ...
    if (flags.length > 0) {
      ...
      for (const flag of flags) {
        ...
        // Show appropriate value placeholder based on flag type
        // WHY: Help text should indicate expected value type for non-boolean flags
        let typeStr = '';
        if (flag.type === 'string') {
          typeStr = ' <value>';
          ...
          // OPTIONAL-STRING: Show [value] to indicate value is optional
          // WHY: Square brackets are Unix convention for optional arguments
          typeStr = ' [value]';
        }
    ...
    console.log('  SVG_BBOX_BROWSER_PATH            Path to Chrome/Chromium executable');
    console.log('  SVG_BBOX_SKIP_BROWSER_DOWNLOAD   Set to "1" to disable auto-download');
    console.log('  PUPPETEER_EXECUTABLE_PATH        Puppeteer native browser path override');
  }
