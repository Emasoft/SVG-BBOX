...
/**
 * **The unified FBF.SVG frame extractor every CLI tool and library
 * consumer should use.**
 *
 * Pass the FBF.SVG markup (a string — must be FBF.SVG, i.e. produced by
 * [svg2fbf](https://github.com/Emasoft/svg2fbf)) and a 1-based frame
 * number, and receive a pinned-frame SVG ready to feed any normal SVG
 * renderer (puppeteer, inkscape, sharp, the browser, …).
 *
 * This is the single entry point all tools share. Compared to the
 * lower-level {@link pinFrame}, it adds:
 *   1. Argument-shape validation with self-explanatory error messages.
 *   2. A clear "input is not an FBF.SVG" branch that names the URL of
 *      the producing tool (svg2fbf), so a user who passes the wrong
 *      file gets a one-line explanation of what FBF.SVG even is.
 *
 * Pure string manipulation — works in the browser, in Puppeteer, and
 * in any Node script without filesystem access. CLI tools that already
 * load their input from disk should pass the loaded string directly.
 *
 * @param {string} svgContent - The full FBF.SVG markup.
 * @param {number} frameNumber - 1-based frame number to extract.
 * @returns {{ svg: string, frameId: string, frameNumber: number, totalFrames: number }}
 *   `svg` is the pinned SVG ready to render; `frameId` is the canonical
 *   FRAMEnnnnn id selected; `frameNumber` echoes the requested 1-based
 *   number; `totalFrames` is the count of frames in the source FBF.SVG.
 */
