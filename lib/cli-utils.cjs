...
function writeJSONOutput(data, outputPath, options = {}) {
  ...
  try {
    ...
    throw new Error(`Failed to serialize data to JSON: ${err.message}`);
  }
  ...
  if (trimmedPath === '-') {
    try {
      ...
      throw new (require('./security-utils.cjs').FileSystemError)(
        `Failed to write JSON to stdout: ${err.message}`,
        { code: err.code }
      );
    }
  ...
  if (!effectiveAllowedDirs) {
    ...
    try {
      ...
      printWarning(`Cannot access current directory: ${err.message}`);
    }
    ...
    try {
      ...
      printWarning(`Cannot access temp directory: ${err.message}`);
    }
