/*! SvgVisualBBox.js v1.2.0 | MIT License | https://github.com/Emasoft/SVG-BBOX */
...
`+"  • Try simplifying the SVG to isolate the problematic element"))};X.onload=N,X.onerror=W}),typeof X.decode==="function")try{await X.decode()}catch{}if(B=document.createElement("canvas"),B.width=F,B.height=Y,f=B.getContext("2d"),!f)throw Error("Failed to get canvas 2D context");f.clearRect(0,0,F,Y),f.drawImage(X,0,0,F,Y)}finally{URL.revokeObjectURL(A),X.src="",X.onload=null,X.onerror=null}let R;try{if(!f)throw Error("Canvas context is null");R=f.getImageData(0,0,F,Y)}catch(O){let G=O instanceof Error?O.message:"",N=/out of memory/i.test(G),W=/tainted/i.test(G);if(N){let I=n(J),y=s(J,K);throw Error(`❌ Cannot render SVG: Canvas out of memory
`+`
ELEMENT DETAILS:
  ${I}
  SVG Root: ${K.id?`id="${K.id}"`:"(no id)"}
...
`+"  • If passing an element reference, make sure it's not null/undefined")}let L=c.get(z);if(L)await L;let H=(F)=>{},P=new Promise((F)=>{H=F});c.set(z,P);try{let F=z.ownerDocument||document;await JJ(F,_);let Y=z.ownerSVGElement||(z instanceof SVGSVGElement?z:null);if(!Y){let k=n(z),U=z&&z.id&&r(z.id)?`
...
`+"  • Ensure the array is not empty before calling this function");let Q=[],Z=null;for(let z=0;z<J.length;z++){let L=J[z];if(!L)continue;let H=await l(L,K);if(!H)continue;if(!Z)Z=H.svgRoot;else if(H.svgRoot!==Z){let P=Z.id?`id="${Z.id}"`:"(no id)",F=H.svgRoot.id?`id="${H.svgRoot.id}"`:"(no id)";throw Error(`Cannot compute union bounding box: Elements from different SVG documents
