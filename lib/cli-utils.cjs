 ...
 * @property {'boolean'|'string'|'number'|'optional-string'} type - Flag type
 * For 'optional-string': returns true if flag present with no value, string if value given
 * @property {string} [description] - Flag description for help text
 * @property {string} [valueLabel] - Override for the help-text value placeholder
 *   (e.g. '<1-255>' instead of the default '<value>'). Display-only — does NOT
 *   change parsing behavior.
 * @property {boolean} [advanced] - Group this flag under "Advanced options" in help.
 *   Use for power-user knobs whose defaults are usually fine.
...
const helpFormatter = require('./help-formatter.cjs');
 ...
 * @param {string} config.description - Command description (one-line tagline)
 * @param {string|string[]} config.usage - Usage string or array of usage lines
 ...
 * @param {string} [config.tagline] - Short tagline shown in the help-screen header box
 *   (defaults to `description` if omitted).
 * @param {string} [config.longDescription] - Multi-paragraph DESCRIPTION block
 *   shown beneath the header box.
 * @param {Array<{title: string, command: string|string[]}>} [config.examples]
 *   Worked examples shown in the EXAMPLES section.
 * @param {{flag: string, argLabel: string, formatBody: string, examples?: string[]}} [config.batch]
 *   Batch-mode documentation (rendered as the BATCH MODE section).
 * @param {{flags: Array<{flag: string, description: string}>}} [config.fbf]
 *   FBF.SVG flag documentation (rendered as the FBF.SVG SUPPORT section).
 * @param {{inkscape?: boolean, extra?: Array<[string, string]>}} [config.environment]
 *   Override or extend the ENVIRONMENT section.
 * @param {Array<[number, string]>} [config.exitCodes] - Exit-code reference table.
 * @param {string} [config.notes] - Free-form NOTES section.
...
function createArgParser(config) {
  ...
  /**
   * Prints help message and exits. Delegates to the unified help formatter
   * so every CLI tool has the same branded layout.
   */
  function printHelp() {
    const usageLines = Array.isArray(usage) ? usage : [usage];
    /** @type {*} */
    const cfg = config;
    console.log(
      helpFormatter.renderHelp({
        toolName: name,
        tagline: cfg.tagline || description,
        description: cfg.longDescription || undefined,
        usage: usageLines,
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (flag) {
        if (flag.type === 'boolean') {
          ...
          // NUMBER TYPE: Parse next argument as a number
          // WHY: OptionDef typedef supports 'number' type for numeric flags like --margin, --timeout
          // Consistent with createModeArgParser's parseFlagValue() number handling (lines 495-499)
          if (i + 1 >= args.length) {
            throw new Error(`Missing value for flag: ${arg}`);
          }
          i++;
          const numValue = parseFloat(/** @type {string} */ (args[i]));
          // VALIDATION: Reject NaN values with clear error message
          // WHY: Silent NaN propagation causes confusing downstream errors
          if (isNaN(numValue)) {
            throw new Error(`Invalid number for --${flag.name}: "${args[i]}"`);
          }
          result.flags[flag.name] = numValue;
        } else if (flag.type === 'optional-string') {
          // OPTIONAL-STRING TYPE: Value is optional
          // WHY: Supports flags like --json that can be used as:
          //   --json (no value) → returns true (auto-generate filename)
          //   --json output.json → returns "output.json" (specified path)
          //   --json - → returns "-" (stdout)
          const nextArg = args[i + 1];
          // Check if next arg is a value (not a flag)
          // WHY: `-` alone means stdout (Unix convention), but `--flag` or `-x` are flags
          const isValue = nextArg !== undefined && (nextArg === '-' || !nextArg.startsWith('-'));
          if (isValue) {
            // Next arg is a value, use it
            i++;
            result.flags[flag.name] = /** @type {string} */ (nextArg);
          } else {
            // No value provided, set to true to indicate flag was present
            result.flags[flag.name] = true;
          }
        }
 ...
 * @param {string} [config.tagline] - Header-box tagline (defaults to `description`).
 * @param {string} [config.longDescription] - DESCRIPTION block beneath the header.
 * @param {Array<{title: string, command: string|string[]}>} [config.examples] - EXAMPLES section.
 * @param {string} [config.modeFlagsHelp] - Free-form text appended after the modes table.
 * @param {{flag: string, argLabel: string, formatBody: string, examples?: string[]}} [config.batch]
 * @param {{flags: Array<{flag: string, description: string}>}} [config.fbf]
 * @param {{inkscape?: boolean, extra?: Array<[string, string]>}} [config.environment]
 * @param {Array<[number, string]>} [config.exitCodes]
...
function createModeArgParser(config) {
  ...
  function detectModeFromFlags(args) {
    for (let i = 0; i < args.length; i++) {
      ...
      if (typeof modeFlagConfig === 'string') {
        ...
        // EXCEPTION: If next arg is --help or -h, don't consume it (allow mode-specific help)
        if (modeFlagConfig.consumesValue) {
          if (i + 1 >= args.length) {
            throw new Error(`Missing value for mode flag: ${arg}`);
          }

          const nextArg = /** @type {string} */ (args[i + 1]);

          // Check if user is requesting mode-specific help (e.g., --extract --help)
          // Don't consume --help/-h as a value, let it be processed for mode help
          if (nextArg === '--help' || nextArg === '-h') {
            // Don't consume --help, just return the mode
            // The main parser will detect --help in modeArgs and show mode help
            return {
              mode,
              flagsConsumed,
              additionalFlags
          ...
          flagsConsumed.push(nextArg);

          // Store the value in the specified target
          if (modeFlagConfig.valueTarget) {
            additionalFlags[modeFlagConfig.valueTarget] = nextArg;
          }
        }
      }
  ...
  /**
   * Prints general help showing all available modes and global options.
   * Called when user runs the command without arguments or with --help.
   * @returns {void}
   */
  function printGeneralHelp() {
    ...
    // Build modes dict with trigger-flag annotation appended to each
    // description so multi-mode tools still show their --batch / --extract
    // trigger flags inline.
    /** @type {Object<string, {description: string}>} */
    const modesForFormatter = {};
    for (const modeName of Object.keys(modes).sort()) {
      ...
      if (Object.keys(modeFlags).length > 0) {
        const triggerFlags = Object.entries(modeFlags)
          .filter(([, mc]) => {
            const configMode = typeof mc === 'string' ? mc : mc.mode;
      ...
      modesForFormatter[modeName] = {
        description: `${modeConfig.description}${isDefault}${flagTrigger}`
      };
    }

    /** @type {string[]} */
    const usageLines =
      Object.keys(modeFlags).length > 0
        ? [`${name} [options] <arguments>`, `${name} --<mode-flag> [options] [arguments]`]
        : [
            `${name} <mode> [options] [arguments]`,
            `${name} <mode> --help   (for mode-specific help)`
          ];
    ...
    console.log(
      helpFormatter.renderHelp({
        ...
        usage: usageLines,
        examples: cfg.examples,
        modes: Object.keys(modesForFormatter).length > 0 ? modesForFormatter : undefined,
        modeFlagsHelp: cfg.modeFlagsHelp,
        commonOptions: helpFormatter.DEFAULT_COMMON_OPTIONS,
        options: globalFlags,
        batch: cfg.batch,
        fbf: cfg.fbf,
  ...
  function printModeHelp(modeName) {
    ...
    if (allFlags.length > 0) usageStr += ' [options]';
    ...
    // Format positional args as a NOTE section after the table-formatted layout.
    let positionalNote = '';
    if (mode.positional && mode.positional.length > 0) {
      const lines = [];
      ...
      for (const pos of mode.positional) {
        ...
        lines.push(`  ${pos.name}${padding}${desc} ${requiredStr}`);
      }
      positionalNote = `Positional arguments:\n${lines.join('\n')}`;
    }

    console.log(
      helpFormatter.renderHelp({
        toolName: `${name} ${modeName}`,
        tagline: mode.description,
        description: positionalNote || undefined,
        usage: [usageStr],
        commonOptions: helpFormatter.DEFAULT_COMMON_OPTIONS,
        options: [...modeSpecificFlags, ...globalFlags],
        environment: cfg.environment,
        exitCodes: cfg.exitCodes
      })
    );
  ...
  /**
   * Prints version information for the CLI tool.
   * Uses getVersion() from version.cjs for consistency.
   * @returns {void}
   */
  function printVersion() {
...
/**
 * Path to the ASCII art banner file
 * WHY: Centralized banner location for all CLI tools
 */
const BANNER_PATH = path.join(__dirname, '..', 'assets', 'svg-bbox_logo_txt.txt');

/**
 * Cached banner content to avoid repeated file reads
 * @type {string|null}
 */
 ...
 * WHY: Display version alongside banner for user context
...
/**
 * Prints the SVG-BBOX ASCII art banner with tool name and version.
 *
 * WHY THIS FUNCTION EXISTS:
 * All CLI tools should display a consistent branded banner at startup.
 * This provides visual identity and shows version for troubleshooting.
 *
 * BEHAVIOR:
 * - Prints banner to stderr (so stdout remains clean for piping)
 * - Shows tool name centered below the banner
 * - Shows version number
 * - Skipped entirely in quiet mode or non-TTY (piping)
 *
 * @param {string} toolName - Name of the CLI tool (e.g., 'sbb-getbbox')
 * @param {Object} [options={}] - Configuration options
 * @param {boolean} [options.quiet=false] - Skip banner in quiet mode
 * @param {boolean} [options.json=false] - Skip banner in JSON output mode
 * @param {boolean} [options.forceShow=false] - Show even if not TTY
 * @returns {void}
 *
 * @example
 * // At start of CLI tool
 * printBanner('sbb-getbbox', { quiet: flags.quiet, json: flags.json });
 */
function printBanner(toolName, options = {}) {
  const { quiet = false, json = false, forceShow = false } = options;

  // WHY: Skip banner in quiet/json modes or when not a TTY (piping)
  // Banner is informational only and should not pollute machine-readable output
  if (quiet || json) return;
  if (!forceShow && !process.stderr.isTTY) return;

  // Load banner from file (cached after first read)
  if (cachedBanner === null) {
    try {
      cachedBanner = fs.readFileSync(BANNER_PATH, 'utf8');
    } catch {
      // WHY: Banner file missing is not fatal - just skip the banner
      cachedBanner = '';
    }
  }
  ...
  // Print banner to stderr so stdout remains clean for piping
  console.error('');
  console.error(cachedBanner);
  console.error(`  ${toolName} v${version}`);
  console.error('');
}
...
function wrapError(originalError, context) {
  ...
  // WHY: Both parameters are required for proper error wrapping
  if (typeof context !== 'string' || !context) {
    context = 'Unknown context';
  }
  // VALIDATION: Check for null/undefined error object
  // WHY: Accessing .message on null/undefined causes TypeError
  // Include context even when error is null to preserve debugging information
  if (!originalError) return new Error(`${context}: Unknown error`);

  // SAFETY: Handle non-Error objects (e.g., thrown strings, numbers)
  // WHY: Accessing .message on non-Error objects returns undefined
  const errorMessage =
    originalError instanceof Error ? originalError.message : String(originalError);
  const wrappedMessage = `${context}: ${errorMessage}`;
...
/**
 * Comprehensive CLI commands reference table.
 * Used when users accidentally run this module directly or need command help.
 */
const CLI_COMMANDS_REFERENCE = `
╔══════════════════════════════════════════════════════════════════════════════╗
║                         SVG-BBOX CLI COMMANDS REFERENCE                      ║
╚══════════════════════════════════════════════════════════════════════════════╝

This toolkit provides 12 command-line tools for SVG bounding box operations.
Run each tool with --help for detailed usage information.

┌─────────────────────────────────────────────────────────────────────────────┐
│ CORE TOOLS (Our Visual BBox Algorithm - Recommended)                       │
├──────────────────────┬──────────────────────────────────────────────────────┤
│ sbb-getbbox          │ Get pixel-accurate visual bounding box              │
│                      │ Example: npx sbb-getbbox input.svg --json           │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-extract          │ List/rename/extract/export SVG objects              │
│                      │ Example: npx sbb-extract input.svg --list           │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-svg2png          │ Render SVG to PNG with accurate bbox                │
│                      │ Example: npx sbb-svg2png input.svg output.png       │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-fix-viewbox      │ Repair missing/broken viewBox attributes            │
│                      │ Example: npx sbb-fix-viewbox broken.svg fixed.svg   │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-compare          │ Visual diff between two SVG files                   │
│                      │ Example: npx sbb-compare a.svg b.svg diff.png       │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-test             │ Test bbox accuracy across methods                   │
│                      │ Example: npx sbb-test input.svg --verbose           │
└──────────────────────┴──────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│ CHROME COMPARISON TOOLS (Chrome's .getBBox() - for comparison)             │
├──────────────────────┬──────────────────────────────────────────────────────┤
│ sbb-chrome-getbbox   │ Get bbox using Chrome's native .getBBox()           │
│                      │ Example: npx sbb-chrome-getbbox input.svg --json    │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-chrome-extract   │ Extract objects using Chrome's .getBBox()           │
│                      │ Example: npx sbb-chrome-extract input.svg --id x    │
└──────────────────────┴──────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│ INKSCAPE TOOLS (Requires Inkscape installed)                               │
├──────────────────────┬──────────────────────────────────────────────────────┤
│ sbb-inkscape-getbbox │ Get bbox using Inkscape CLI                         │
│                      │ Example: npx sbb-inkscape-getbbox input.svg --json  │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-inkscape-extract │ Extract by ID using Inkscape                        │
│                      │ Example: npx sbb-inkscape-extract input.svg --id x  │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-inkscape-text2path│ Convert text elements to paths                     │
│                      │ Example: npx sbb-inkscape-text2path in.svg out.svg  │
├──────────────────────┼──────────────────────────────────────────────────────┤
│ sbb-inkscape-svg2png │ Export SVG to PNG using Inkscape                    │
│                      │ Example: npx sbb-inkscape-svg2png in.svg out.png    │
└──────────────────────┴──────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│ MAIN HELP COMMAND                                                          │
├──────────────────────┬──────────────────────────────────────────────────────┤
│ svg-bbox             │ Interactive help menu for all tools                 │
│                      │ Example: npx svg-bbox --help                        │
└──────────────────────┴──────────────────────────────────────────────────────┘

NAMING CONVENTION:
  sbb-<function>           Our reliable visual bbox algorithm
  sbb-chrome-<function>    Chrome's .getBBox() method (for comparison)
  sbb-inkscape-<function>  Inkscape tools (requires Inkscape installed)
...
/**
 * Prints the CLI commands reference to stdout.
 * Can be used by any script to show available commands.
 */
function printCLIReference() {
  console.log(CLI_COMMANDS_REFERENCE);
}
...
// WHY: Users may accidentally try to run cli-utils.cjs thinking it's a launcher
// This provides helpful guidance instead of confusing errors
...
╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  This file (cli-utils.cjs) is a LIBRARY MODULE, not a command-line tool.
  It provides shared utilities used by the SVG-BBOX CLI tools.

  You cannot run this file directly to use SVG-BBOX functionality.
  Instead, use one of the CLI tools listed below.
`);
    printCLIReference();
    process.exit(0);
  }
  ...
  if (args[0] === '--version' || args[0] === '-v') {
    try {
      const pkg = require('../package.json');
      console.log(`svg-bbox v${pkg.version} (cli-utils module)`);
    } catch {
      console.log('svg-bbox (version unknown)');
    }
    process.exit(0);
  }

  // Any other argument - show error with guidance
  console.error(`
Error: cli-utils.cjs is a library module, not an executable tool.

You tried to run: node lib/cli-utils.cjs ${args.join(' ')}

This module provides shared utilities for the SVG-BBOX CLI tools.
It cannot be used directly as a command-line tool.
`);
  printCLIReference();
  process.exit(1);
}
