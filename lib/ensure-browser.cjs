 ...
 * WHY: Need synchronous offline check for downloadBrowser() retry logic
 * HOW: Use promisified DNS - node keeps event loop running until promise settles
 * @returns {boolean} True if offline
 */
function isOffline() {
  try {
    // Use promisified DNS - node waits for promise to settle before exiting
    // If DNS succeeds, exits with code 0. If fails (offline), .catch() calls exit(1)
    execSync(
      "node -e \"require('dns').promises.resolve('registry.npmjs.org').catch(() => process.exit(1))\"",
      {
        stdio: 'ignore',
        timeout: 5000
      }
...
/**
 * Async delay helper for retry logic
 * WHY: Cleaner than spawning external sleep commands for async callers
...
/**
 * Download Chrome browser for puppeteer with retry logic (async version)
 * Shows progress to user (unless in CI)
 * WHY: Uses async delay for non-blocking retry waits
 * @param {number} [retryCount=0] - Current retry count
 * @returns {Promise<boolean>} True if download succeeded
 */
async function downloadBrowser(retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < MAX_DOWNLOAD_RETRIES) {
      ...
      // Async delay for retry - non-blocking and cross-platform
      // WHY: Cleaner than spawning external processes (sleep/ping) for delay
      await delay(RETRY_DELAY_MS);

      return await downloadBrowser(retryCount + 1);
    }
...
/**
 * Download Chrome browser for puppeteer with retry logic (sync version)
 * Shows progress to user (unless in CI)
 * WHY: Blocking version needed for CLI startup via ensureBrowserSync
 * @param {number} [retryCount=0] - Current retry count
 * @returns {boolean} True if download succeeded
 */
function downloadBrowserSync(retryCount = 0) {
  // Check if download is disabled
  if (isDownloadDisabled()) {
    console.log(
      '\x1b[33m[svg-bbox]\x1b[0m Browser auto-download disabled (SVG_BBOX_SKIP_BROWSER_DOWNLOAD=1)'
  ...
  try {
    ...
    if (retryCount < MAX_DOWNLOAD_RETRIES) {
      ...
      // Synchronous delay for retry using platform-appropriate sleep command
      // WHY: setTimeout is non-blocking - node process exits immediately before callback fires
      // HOW: Use actual blocking sleep commands that wait the full duration
      if (process.platform === 'win32') {
        // Windows: Use ping localhost as sleep alternative (1 ping per second + 1)
        execSync(`ping -n ${Math.ceil(RETRY_DELAY_MS / 1000) + 1} 127.0.0.1 > nul`, {
          stdio: 'ignore'
        });
      } else {
        // Unix/macOS/Linux: Use sleep command with fractional seconds
        execSync(`sleep ${RETRY_DELAY_MS / 1000}`, { stdio: 'ignore' });
      }

      return downloadBrowserSync(retryCount + 1);
    }
 ...
 * WHY: CLI tools need blocking behavior to ensure browser is ready before proceeding
