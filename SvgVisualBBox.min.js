...
`+"  • If passing an element reference, make sure it's not null/undefined")}let N=d.get(z);if(N)await N;let L=(_)=>{},H=new Promise((_)=>{L=_});d.set(z,H);try{let _=z.ownerDocument||document;await JJ(_,C);let O=z.ownerSVGElement||(z instanceof SVGSVGElement?z:null);if(!O){let V=n(z),f=z&&z.id&&r(z.id)?`
...
ELEMENT DETAILS:
  ${V}
`+f+`This element is not connected to an <svg> root element.
...
`+"  • Verify the element hasn't been removed from the document")}let T=O.viewBox&&O.viewBox.baseVal,G;if(T&&T.width&&T.height)G={x:T.x,y:T.y,width:T.width,height:T.height};else{let V=GJ(O);if(V)G={x:V.x,y:V.y,width:V.width,height:V.height};else G={x:-2000,y:-2000,width:4000,height:4000}}let u=[];(function V(f){let b=f.getAttribute&&f.getAttribute("xlink:href"),x=f.getAttribute&&f.getAttribute("href");for(let F of[b,x])if(F&&F.startsWith("#")){let k=F.substring(1),W=O.getElementById(k);if(W&&!u.includes(W))u.push(W)}let h=f.getAttribute&&f.getAttribute("style");if(h){let F=h.match(/url\(#([^)]+)\)/g);if(F)for(let k of F){let W=k.match(/url\(#([^)]+)\)/);if(!W)continue;let y=W[1];if(!y)continue;let B=O.getElementById(y);if(B&&!u.includes(B))u.push(B)}}let Y=f.children;for(let F=0;F<Y.length;F++)V(Y[F])})(z);let q;if(Q==="unclipped"){let{x:V,y:f}=G,b=G.x+G.width,x=G.y+G.height,h=KJ(O,G,O.getBoundingClientRect()),Y=(F)=>{try{let k=F.getBoundingClientRect(),W=O.getBoundingClientRect();if(k.width>0&&k.height>0){let y,B,U,I;if(h.uniform){let w=h.scale;y=G.x+(k.left-W.left-h.offsetX)*w,B=G.y+(k.top-W.top-h.offsetY)*w,U=G.x+(k.right-W.left-h.offsetX)*w,I=G.y+(k.bottom-W.top-h.offsetY)*w}else y=G.x+(k.left-W.left)*h.scaleX,B=G.y+(k.top-W.top)*h.scaleY,U=G.x+(k.right-W.left)*h.scaleX,I=G.y+(k.bottom-W.top)*h.scaleY;let M=200;V=Math.min(V,y-M),f=Math.min(f,B-M),b=Math.max(b,U+M),x=Math.max(x,I+M)}}catch{}};Y(z);for(let F of u)Y(F);q={x:V,y:f,width:b-V,height:x-f}}else q={x:G.x,y:G.y,width:G.width,height:G.height};let D=1;if(E&&G.width>0&&G.height>0){let V=O.getBoundingClientRect();if(V&&V.width>0&&V.height>0){let f=V.width/G.width,b=V.height/G.height;D=(f+b)/2}}let j=Math.max(1,D*Z),X=Math.max(4,D*$),A=await QJ(z,O,q,j);if(!A)return null;let m=S;if(m==null||!isFinite(m)){let V=Math.max(A.width,A.height);m=(V>0?V*0.25:0)+100}let R=A.x-m,v=A.y-m,a=A.x+A.width+m,P=A.y+A.height+m,p={x:R,y:v,width:Math.max(0,a-R),height:Math.max(0,P-v)};if(p.width<=0||p.height<=0)return null;let c=await QJ(z,O,p,X);if(!c)return null;return{x:c.x,y:c.y,width:c.width,height:c.height,element:z,svgRoot:O}}finally{L(),d.delete(z)}}async function ZJ(J,K){if(!Array.isArray(J)||J.length===0)throw Error(`Cannot compute union bounding box: Invalid targets parameter
