...
function getLaunchOptions() {
  ...
  const options = {
    ...
    // Puppeteer's 30s default for CDP RPC calls is too short under
    // pooled-browser load — bump to 120s.
 ...
 *
 * WHY connect-vs-launch: When tests set $SBB_BROWSER_WS, ALL pooled browsers
 * are pages of the SAME shared Chromium (acquired via puppeteer.connect).
 * This way the chrome-pool keeps its existing per-acquire/release semantics
 * but doesn't multiply Chromium processes — peak Chromium count stays at 1
 * regardless of pool size. Production behavior (no env var) is unchanged.
 *
 * @returns {Promise<PooledBrowser>}
 */
async function createBrowser() {
  const browser = process.env.SBB_BROWSER_WS
    ? await puppeteer.connect({
        browserWSEndpoint: process.env.SBB_BROWSER_WS,
        protocolTimeout: PROTOCOL_TIMEOUT_MS
      })
    : await puppeteer.launch(getLaunchOptions());
  const now = Date.now();
...
async function destroyBrowser(pooledBrowser, reason = 'unknown') {
  ...
  // WHY connect-vs-close: in test mode the pooled browser is just a
  // connection to the shared Chromium owned by global-setup. Calling
  // browser.close() would kill the shared instance and break other tests.
  // Detect mode via $SBB_BROWSER_WS and disconnect instead.
  const isConnected =
    process.env.SBB_BROWSER_WS &&
    (() => {
      try {
        return pooledBrowser.browser.wsEndpoint() === process.env.SBB_BROWSER_WS;
      } catch {
        return false;
      }
    })();
  ...
  if (isConnected) {
    try {
      pooledBrowser.browser.disconnect();
    } catch {
